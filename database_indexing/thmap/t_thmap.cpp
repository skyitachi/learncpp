...
test_delete(void)
{
	...
	keys = (uint64_t*)calloc(nitems, sizeof(uint64_t));
	assert(keys != NULL);
...
test_random(void)
{
	...
	keys = (unsigned char **)calloc(nitems, sizeof(unsigned char *));
	assert(keys != NULL);

	lens = (int*)calloc(nitems, sizeof(int));
	assert(lens != NULL);
	...
	while (n--) {
		...
		case 0:
			...
			if (keys[i] == NULL) {
				keys[i] = (unsigned char *)generate_unique_key(i, &lens[i]);
